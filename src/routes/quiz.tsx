...
export const Route = createFileRoute("/quiz")({
  head: () => ({
    meta: [
      { title: "Quiz de avaliação — Forjar" },
...
function QuizPage() {
  ...
  function setNumber(key: "age", rawValue: string) {
    ...
    setAnswers((current) => ({
      ...current,
      [key]: normalized === "" ? undefined : Number(normalized),
    }));
  }
  function toggleArr(key: "equipment" | "limitations" | "priorities", value: string) {
    setAnswers((a) => {
  ...
  const skipEquipment =
    answers.location === "full_gym" ||
    answers.location === "basic_gym" ||
    answers.location === "home_no_equipment";
  ...
  function canContinue(): boolean {
    switch (step) {
      ...
      case 7:
        return skipEquipment || (answers.equipment && answers.equipment.length > 0) || false;
      case 8:
        return !!answers.daysPerWeek;
      case 9:
        return !!answers.minutesPerSession;
      case 10:
        return (
          answers.hasLimitation !== undefined &&
          (!answers.hasLimitation ||
            (answers.limitations || []).length > 0 ||
            !!answers.limitationNotes)
        );
      case 11:
        return true; // priorities optional
      case 12:
        return !!answers.includeCardio;
      case 13:
        return true;
      case 14:
        return !!answers.conditioning;
      case 15:
        return true;
      default:
        return false;
  ...
  async function finish() {
    ...
    try {
      ...
      await stagePromise;
      setProcessing("Validando demonstrações dos exercícios...");
      const verified = await generateWorkoutPlanWithVerifiedMedia(profile, library!);
      ...
      <div className="mx-auto flex min-h-[70vh] max-w-md items-center justify-center px-4">
        <div className="w-full rounded-2xl border border-border bg-card p-6 text-center">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            Ocorreu um problema ao conectar com a base de exercícios. Verifique sua conexão e tente
            novamente.
          </p>
      ...
      <div className="rounded-2xl border border-border bg-card p-6 sm:p-8">
        ...
        {step === 6 && (
          <Q title="Onde você realizará os treinos?">
            <Choices
              ...
              onChange={(v) => set("location", v as UserProfile["location"])}
              options={[
        ...
        {step === 9 && (
          <Q title="Quanto tempo você possui para cada treino?">
            <Choices
              value={String(answers.minutesPerSession)}
          ...
          <Q title="Você possui alguma lesão, dor ou limitação?">
            ...
            {answers.hasLimitation && (
              <div className="mt-4 space-y-3">
                <div className="grid grid-cols-2 gap-2">
                  {LIMITATION_OPTIONS.map((l) => (
                    <button
                      key={l}
                      type="button"
                      onClick={() => toggleArr("limitations", l)}
                ...
                <textarea
                  value={answers.limitationNotes ?? ""}
                  onChange={(e) => set("limitationNotes", e.target.value)}
                  placeholder="Descreva brevemente sua limitação (opcional)"
                  rows={3}
                  className="w-full rounded-xl border border-border bg-background px-4 py-3 text-sm"
                />
                <div className="rounded-lg bg-brand/10 p-3 text-sm text-brand">
                  Antes de iniciar o plano, procure a liberação de um profissional de saúde e a
                  orientação presencial de um profissional de Educação Física.
                </div>
              </div>
            )}
          </Q>
        ...
        {step === 11 && (
          <Q
            title="Quais grupos musculares você deseja priorizar?"
            subtitle="Selecione quantas opções desejar. O treino continuará equilibrado."
          >
            <div className="grid grid-cols-2 gap-2">
              {PRIORITY_OPTIONS.map((p) => (
                <button
                  key={p}
                  type="button"
                  onClick={() => toggleArr("priorities", p)}
        ...
        {step === 12 && (
          <Q title="Você deseja incluir exercícios aeróbicos?">
            <Choices
              value={answers.includeCardio}
              onChange={(v) => set("includeCardio", v as UserProfile["includeCardio"])}
              options={[
                { value: "yes", label: "Sim" },
                { value: "no", label: "Não" },
                { value: "recommended", label: "Apenas se recomendado" },
              ]}
            />
          </Q>
        )}
        {step === 13 && (
          <Q
            title="Existe algum exercício que você não pode ou não gosta de realizar?"
            subtitle="Opcional."
          >
            <textarea
              ...
              rows={3}
              placeholder="ex: burpee, agachamento profundo"
              className="w-full rounded-xl border border-border bg-background px-4 py-3 text-sm"
            />
          </Q>
        )}
        {step === 14 && (
          <Q title="Como você classifica seu condicionamento físico atual?">
            <Choices
              value={answers.conditioning}
              onChange={(v) => set("conditioning", v as UserProfile["conditioning"])}
              options={[
        ...
        {step === 15 && (
          <Q title="Alguma informação importante a considerar?" subtitle="Opcional.">
            <textarea
              ...
              rows={4}
              placeholder="ex: rotina sedentária, retorno após período parado"
              className="w-full rounded-xl border border-border bg-background px-4 py-3 text-sm"
            />
          </Q>
        )}
